...
def user_management():  # this function is responsible for adding users
    def adding_user():
        def adding_admin():
            ...
            username.grid(row=1, column=1)
            admin3 = tk.Button(master=system, text="Confirm Username", width=25, command=admin_username).grid(row=1,column=3)
        ...
        def adding_standard():
            def standard_username():
                ...
                if user in admin or user in manager or user in standard:
                    ...
                    passwrd = tk.Entry(master=system, show='*')
                    passwrd.grid(row=2, column=1)
                    tk.Button(master=system, text="Confirm Password", command=standard_back_users, width=25).grid(row=2,column=3)
            ...
            username = tk.Entry(master=system)
            username.grid(row=1, column=1)
            admin3 = tk.Button(master=system, text="Confirm Username", width=25, command=standard_username).grid(row=1,column=3)

        adding_users_scheduled = tk.Tk()
        adding_users_scheduled.title("Add A User")
        tk.Button(master=adding_users_scheduled, width=25, text="Add An Admin", command=adding_admin).grid(row=1,column=1)
        tk.Button(master=adding_users_scheduled, width=25, text="Add A Supervisor", command=add_manager).grid(row=2,column=1)
        tk.Button(master=adding_users_scheduled, width=25, text="Add A Standard User", command=adding_standard).grid(row=3, column=1)
    ...
    def deleting_users():
        ...
        username.grid(row=1, column=1)
        admin2 = tk.Button(master=delete_users_scheduled, text="Confirm Deletion", width=25,command=deleting_back_user).grid(row=1, column=2)
    ...
    tk.Button(master=manage_user_scheduled, text="Delete A User", width=25, command=deleting_users).grid(row=2,column=0)
    tk.Button(master=manage_user_scheduled, text="View Current Users", width=25, command=display_users).grid(row=3,column=0)
...
def login():  # login function
    ...
    # image = tk.PhotoImage(file="icon.png")
    no = tk.Label(master=login_sched).grid(row=0, column=1)
